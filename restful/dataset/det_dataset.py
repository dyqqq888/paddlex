...
class DetDataset(DatasetBase):
    ...
    def check_dataset(self, endpoint,folder):
        minio_conf = {
            'endpoint': endpoint,
            'access_key': 'admin',
            'secret_key': '12345678',
        ...
        if not client.bucket_exists('datasets'):
            return None
        data=client.list_objects('datasets')

        
        
        data=[]
        index=1
        path="/home/www/work/code/PaddleX/paddlex_restful/Workspace/JPEGImages"
        while index:
            try:
                data =client.get_object('datasets','/yinyingzhizhao/JPEGImages'+'/'+str(index-1)+'.jpg')
                with open(path+'/'+str(index-1)+'.jpg', 'wb') as file_data:
        ...
        """
        self.all_files = list_files(path)
        # 对检测数据集进行统计分析
        self.file_info = dict()
        self.label_info = dict()

        if osp.exists(osp.join(path, 'train_list.txt')):
            return self.check_splited_dataset(path)

        for f in self.all_files:
            if not is_pic(f):
                continue
            items = osp.split(path+'/'+f)
            if len(items) == 2 and items[0] == "/home/www/work/code/PaddleX/paddlex_restful/Workspace/JPEGImages":
