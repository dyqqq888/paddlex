...
#图像分类
class ClsDataset(DatasetBase_minio):
    def __init__(self, dataset_id, path):
        super().__init__(dataset_id, path)
    
    def check_dataset(self, endpoint,folder,dataset_path):
        minio_conf = {
            'endpoint': endpoint,
            'access_key': 'admin',
            'secret_key': '12345678',
        ...
        #folder = os.path.exists(dataset_path+"/Annotations")
        if not os.path.exists(dataset_path+"/Annotations"):
            os.mkdir(dataset_path+"/Annotations")
        index=1
        #path="/home/www/work/code/PaddleX/paddlex_restful/Workspace/"
        while index:
            try:
                data =client.get_object('datasets','/yinyingzhizhao/JPEGImages'+'/'+str(index-1)+'.jpg')
                #with open(path+'JPEGImages/'+str(index-1)+'.jpg', 'wb') as file_data:
                with open(dataset_path+'/'+str(index-1)+'.jpg', 'wb') as file_data:
                    for d in data.stream(32 * 1024):
                        file_data.write(d)
                

                data2 =client.get_object('datasets','/yinyingzhizhao/Annotations'+'/'+str(index-1)+'.xml')
                #with open(path+'Annotations/'+str(index-1)+'.xml', 'wb') as file_data:
                with open(dataset_path+'/Annotations/'+str(index-1)+'.xml', 'wb') as file_data:
        ...
        for f in self.all_files:
            if not is_pic(f):
                continue
            items = osp.split(dataset_path+'/'+f)
            if len(items) == 2:
                if " " in items[0]:
