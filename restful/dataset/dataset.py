...
def list_datasets(workspace):
    ...
    """
    from paddlex_restful.restful.dataset.operate import get_dataset_status
    dataset_list = list()
...
def get_dataset_details(data, workspace):
    ...
    Return:
        ...
        'test_files': 测试集文件列表
    """
    from paddlex_restful.restful.dataset.operate import get_dataset_details
    dataset_id = data['did']
    assert dataset_id in workspace.datasets, "数据集ID'{}'不存在.".format(dataset_id)
    dataset_path = workspace.datasets[dataset_id].path
